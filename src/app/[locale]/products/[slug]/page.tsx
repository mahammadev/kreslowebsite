...
import { FlashSaleCountdown } from '@/components/product/FlashSaleBadge';
...
export default async function ProductPage(props: { params: Promise<{ locale: string; slug: string }> }) {
    ...
    const isOnSale = product.discount_price && product.discount_ends_at && new Date(product.discount_ends_at) > new Date();
                ...
                <div className="flex sticky top-0 flex-col col-span-5 2xl:col-span-4 max-md:col-span-full md:h-screen min-h-max md:py-24 max-md:py-8 max-md:static">
                    <div className="col-span-full">
                        ...
                        <div className="flex flex-col col-span-full gap-4 md:mb-10 max-md:order-2">
                            <div className="flex flex-col px-4 py-6 rounded-md bg-white border border-zinc-200 md:gap-y-6 place-items-baseline">
                                <h1 className="text-2xl font-bold lg:text-3xl 2xl:text-4xl text-balance max-md:mb-4">
                                    {localizedName}
                                </h1>

                                {/* Flash Sale Countdown */}
                                {isOnSale && product.discount_ends_at && (
                                    <div className="w-full mb-4">
                                        <FlashSaleCountdown endsAt={product.discount_ends_at} locale={locale} />
                                    </div>
                                )}

                                <p className="flex gap-3 items-center text-xl font-bold lg:text-3xl 2xl:text-4xl max-md:mt-4 tracking-tight">
                                    {product.price_negotiable ? (
                                        <span className="text-xl lg:text-3xl text-primary">{t('negotiable')}</span>
                                    ) : displayDiscount ? (
                                        <>
                                            <span className="text-red-600">{displayDiscount}</span>
                            ...
                            {/* Product Actions (Add to Cart + WhatsApp) */}
                            <ProductActions
