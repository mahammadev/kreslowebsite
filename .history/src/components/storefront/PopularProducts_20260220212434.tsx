...
export const PopularProducts = async () => {
    ...
    return (
        <section>
            {/* Product grid — template exact: 1→2→3 columns, square cards, hover overlay */}
                ...
                {products.map((product) => {
                    ...
                    return (
                        <div key={product.id} className="relative w-full aspect-[3/4] md:aspect-square bg-muted group overflow-hidden">
                            ...
                            {/* Interactive overlay — exactly like template */}
                            <div className="absolute inset-0 p-2 w-full pointer-events-none">
                                {/* Top bar: title + price — visible normally, hides on hover */}
                                ...
                                {/* Bottom panel: slides up on hover */}
