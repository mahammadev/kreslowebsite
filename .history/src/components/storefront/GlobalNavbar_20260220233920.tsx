...
export const GlobalNavbar = async ({ locale }: { locale: string }) => {
    ...
    const siteTitle = settings.site_title || "KRESLO";
    ...
    return (
        <header className="grid fixed top-0 left-0 z-50 grid-cols-3 items-start w-full p-sides md:grid-cols-12 md:gap-sides">
            ...
            {/* Logo / Brand */}
            <Link href={`/${locale}`} className="md:col-span-3 xl:col-span-2 flex items-center justify-start md:h-16" prefetch>
                {logoUrl ? (
                    <div className="relative h-12 md:h-16 w-48 md:w-64 max-md:mx-auto">
                        <Image src={logoUrl} alt={siteTitle} fill className="object-contain object-left max-md:object-center" priority sizes="256px" />
                    </div>
                ) : (
