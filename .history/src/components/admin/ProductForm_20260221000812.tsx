...
export function ProductForm({ mode, product, categories }: ProductFormProps) {
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      ...
      colors: (product?.attributes?.colors?.join(', ') || '') as string,
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    const parsedColors = values.colors
      ? values.colors.split(',').map(c => c.trim()).filter(Boolean)
      : [];
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8 py-4">
            ...
            <FormField control={form.control} name="colors" render={({ field }) => (
              <FormItem>
                ...
                <FormControl><Input placeholder="e.g. Qırmızı, Qara, Göy" {...field} value={field.value ?? ''} /></FormControl>
                <FormDescription>Comma-separated list of colors available for this product.</FormDescription>
                <FormMessage />
              </FormItem>
            )} />
