...
const PREDEFINED_COLORS = [
  'Qırmızı', 'Mavi', 'Göy', 'Yaşıl', 'Sarı', 'Narıncı',
  'Bənövşəyi', 'Çəhrayı', 'Qara', 'Ağ', 'Boz', 'Qəhvəyi',
  'Gümüşü', 'Qızılı', 'Bej',
  'Qara/Qırmızı', 'Qara/Ağ', 'Boz/Qara', 'Boz/Ağ', 'Qəhvəyi/Bej'
];
...
export function ProductForm({ mode, product, categories }: ProductFormProps) {
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      ...
      extra_images: (product?.extra_images || []) as string[],
      colors: (product?.attributes?.colors || []) as string[],
      is_active: (product?.is_active ?? true) as boolean,
      in_stock: (product?.in_stock ?? true) as boolean,
      price_negotiable: (product?.price_negotiable ?? false) as boolean,
    },
  });
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8 py-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <FormField control={form.control} name="discount_price" render={({ field }) => (
                <FormItem><FormLabel>Discount Price <i>(Optional)</i></FormLabel><FormControl><Input type="number" {...field} value={field.value ?? ''} disabled={form.watch('price_negotiable')} /></FormControl><FormMessage /></FormItem>
              )} />
            </div>
            ...
            <FormField control={form.control} name="colors" render={({ field }) => (
              <FormItem>
                <FormLabel>Colors <i>(Optional)</i></FormLabel>
                <FormControl>
                  <div className="flex flex-wrap gap-3 p-4 border rounded-md bg-muted/20">
                    {PREDEFINED_COLORS.map(colorName => {
                      const hex = getColorHex(colorName);
                      const isSelected = field.value?.includes(colorName);
                        ...
                        <div
                          ...
                          onClick={() => {
                            ...
                            const newValue = isSelected
                              ...
                              : [...current, colorName];
                            form.setValue('colors', newValue, { shouldDirty: true });
                          }}
                        >
                          <ColorSwatch
                            ...
                            size="lg"
                            active={isSelected}
                            className={isSelected ? "ring-offset-background" : "opacity-60 hover:opacity-100"}
                          />
                          <span className={`text-[10px] uppercase ${isSelected ? 'font-bold text-foreground' : 'text-muted-foreground'}`}>
                            {colorName}
                          </span>
                        </div>
                      );
                ...
                <FormDescription>Select the available colors for this product.</FormDescription>
                <FormMessage />
              </FormItem>
            )} />
